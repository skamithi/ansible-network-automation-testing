...
class Interfaces(FactsBase):
    ...
    def populate(self):
        ...
        for item in reply[0]:
            ...
            obj = {
                'oper_status': self.get_text(item, 'oper-status'),
                'admin_status': self.get_text(item, 'admin-status'),
            ...
            for liface in item.iter('logical-interface'):
                liface_name = self.get_text(liface, 'name')
                obj['interface_address'] = []
                liface_broadcast = self.get_text(liface, 'address-family/interface-address/ifa-destination')
                if liface_broadcast and liface_broadcast != 'Unspecified':
                    liface_subnet = liface_broadcast.split('/')[1]
                    liface_local = self.get_text(liface, 'address-family/interface-address/ifa-local')
                    obj['interface_address'].append( {
                        "name": liface_name,
                        "ip": "%s/%s" % (liface_local, liface_subnet),
                        "address_family": self.get_text(liface, 'address-family/address-family-name')
                    })
